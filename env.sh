...
deps() { sudo apt-get install cmake autoconf libavutil-dev libswresample-dev libavcodec-dev libavformat-dev libmicrohttpd-dev libssl-dev && ./build_ffmpeg.sh && rm -rf deps && mkdir deps && cd deps && wget http://www.digip.org/jansson/releases/jansson-2.12.tar.gz && tar xvf jansson-2.12.tar.gz && cd jansson-2.12 && autoreconf -fi && ./configure &&  make && make check && sudo make install && cd ../ && git clone https://github.com/warmcat/libwebsockets && cd libwebsockets && mkdir build && cd build && cmake .. && make && sudo make install && cd ../../../; }
